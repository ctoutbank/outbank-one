...
import { customers, merchantPriceGroup, merchantTransactionPrice, settlements, paymentInstitution, paymentLink, shoppingItems, reports, reportFilters, reportFiltersParam, solicitationFee, solicitationFeeBrand, merchants, merchantfile, file, userMerchants, users, solicitationFeeDocument, fee, feeBrand, profiles, profileFunctions, functions, payoutAntecipations, userNotifications, salesAgents, modules, moduleFunctions, categories, legalNatures, configurations, addresses, merchantPrice, merchantBankAccounts, fornecedores, fornecedorCategories, solicitationBrandProductType, financialAdjustments, financialAdjustmentMerchants, feeBrandProductType, merchantpixaccount, feeCredit, customerFunctions, payout, contacts, customerCustomization, merchantSettlements, merchantSettlementOrders, reportExecution, reportExecutionStatus, fornecedorDocuments, merchantPixSettlementOrders } from "./schema";
...
export const customersRelations = relations(customers, ({one, many}) => ({
	...
	fees: many(fee),
	financialAdjustments: many(financialAdjustments),
	users: many(users),
...
export const feeRelations = relations(fee, ({one, many}) => ({
	feeBrands: many(feeBrand),
	customer: one(customers, {
		fields: [fee.idCustomer],
		references: [customers.id]
	}),
}));
...
export const categoriesRelations = relations(categories, ({many}) => ({
	merchants: many(merchants),
	fornecedorCategories: many(fornecedorCategories),
}));
...
export const fornecedorCategoriesRelations = relations(fornecedorCategories, ({one}) => ({
	fornecedore: one(fornecedores, {
		fields: [fornecedorCategories.fornecedorId],
		references: [fornecedores.id]
	}),
	category: one(categories, {
		fields: [fornecedorCategories.categoryId],
		references: [categories.id]
	}),
}));
...
export const fornecedoresRelations = relations(fornecedores, ({many}) => ({
	fornecedorCategories: many(fornecedorCategories),
	fornecedorDocuments: many(fornecedorDocuments),
}));
...
export const financialAdjustmentsRelations = relations(financialAdjustments, ({one, many}) => ({
	financialAdjustmentMerchants: many(financialAdjustmentMerchants),
	customer: one(customers, {
		fields: [financialAdjustments.idCustomer],
		references: [customers.id]
	}),
}));
...
export const fornecedorDocumentsRelations = relations(fornecedorDocuments, ({one}) => ({
	fornecedore: one(fornecedores, {
		fields: [fornecedorDocuments.fornecedorId],
		references: [fornecedores.id]
	}),
}));
