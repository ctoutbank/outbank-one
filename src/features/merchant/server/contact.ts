...
export async function getContactByMerchantId(id: number) {
    ...
    }).from(contacts).where(eq(contacts.idMerchant, id)).leftJoin(addresses, eq(contacts.idAddress, addresses.id));
...
export async function getContactByMerchantId2(id: number): Promise<Array<{
