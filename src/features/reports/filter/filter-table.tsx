...
export default function FilterTableAndForm({ filter, reportId, reportFilterParams, reportTypeDD }: FilterTableAndFormProps) {
    ...
    function handleNewReportFilter() {
        setRecord({
            idReport: reportId,
            idReportFilterParam: filteredParams.length > 0 ? filteredParams[0].id : reportFilterParams[0]?.id,
            value: selectedType || '',
            typeName: reportTypeDD.find(type => type.code === selectedType)?.name || ''
        });
        setOpen(true);
    }
    ...
    const isBrandFilter = (record: ReportFilterDetailWithTypeName): boolean => {
        const param = reportFilterParams.find(p => p.id === record.idReportFilterParam);
        return param?.name === 'Bandeira';
    };

    // Função para verificar se um filtro é de intervalo de datas (totalAmount do tipo Vendas)
    const isDateRangeFilter = (record: ReportFilterDetailWithTypeName): boolean => {
        const param = reportFilterParams.find(p => p.id === record.idReportFilterParam);
    ...
    const isValueRangeFilter = (record: ReportFilterDetailWithTypeName): boolean => {
        const param = reportFilterParams.find(p => p.id === record.idReportFilterParam);
        return param?.name === 'valor' && record.typeName === 'Vendas';
    };

    // Função para verificar se um filtro é de Estabelecimento
    const isMerchantFilter = (record: ReportFilterDetailWithTypeName): boolean => {
        const param = reportFilterParams.find(p => p.id === record.idReportFilterParam);
    ...
    const formatValueRange = (valueString: string): string => {
        ...
        return `De ${parseFloat(min).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} até ${parseFloat(max).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`;
    };
    ...
    const renderFilterValue = (record: ReportFilterDetailWithTypeName): React.ReactNode => {
        // Verificar se é filtro de bandeiras do tipo AL
        if (isBrandFilter(record)) {
            // Mostrar lista separada por vírgulas
            return record.value.split(',').map(b => b.trim()).join(', ');
        }
        ...
        if (isStatusFilter(record)) {
            ...
            return Status.find(s => s.value === record.value)?.label || record.value;
        }
            ...
            <div className="basis-1/2 text-left px-7">
                <Dialog open={open} onOpenChange={setOpen}>
                    <DialogTrigger onClick={handleNewReportFilter}>
                        <div className="mb-4 h-8 px-4 py-2 inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 float-right">
                            Novo Filtro
                        </div>
                    </DialogTrigger>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>{record?.id ? 'Editar Filtro' : 'Novo Filtro'}</DialogTitle>
