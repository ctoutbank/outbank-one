...
export async function GET() {
  ...
  await createCronJobMonitoring({
    jobName: "Sincronização de settlements",
    status: "pending",
    startTime: new Date().toISOString(),
  });
  try {
    console.log("Iniciando sincronização de settlements...");
    ...
    const successResponse = NextResponse.json({
      message: "Sincronização de settlements concluída com sucesso",
    });
    ...
    const errorResponse = NextResponse.json(
      {
        error: "Erro na sincronização de settlements",
